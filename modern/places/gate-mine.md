---
name: Gate Golf — the Mine Gate
type: location
category: places
tags:
  - portal
  - containment
  - unreachable
  - siberia
  - permafrost
  - mine
related:
  - ../lore/portals
  - ../factions/joint-portal-command
  - the-ural-scar
  - gate-ice
summary: "The portal a Siberian mine broke into — sealed under a collapsed shaft, still humming, and refusing to die."
---

# Gate Golf — the Mine Gate

> They buried the door. The door did not stop.

## Overview

**Gate Golf** — JPC designation **G-07**, known as **the Mine Gate** — is a portal the human race broke into by accident. The site is a permafrost mine in eastern Siberia, worked for a century for minerals that no longer matter. On Day One, a deep heading broke through into a **buried shaft** — a vertical passage lined with not-stone, descending past the depth of any survey. The hordes came up through the workings. The mine became a battlefield, then a graveyard, then a sealed shaft. It is the second Russian gate — and the one the charter does not see: the site is closed to all but Russian personnel, and JPC's containment of G-07 rests on Moscow's word. Whether the shaft behind the seal is still a grave is the question nobody in the command asks out loud (see [the unknown doors](../factions/nations.md#the-unknown-doors)).

The Scouring's attempt to destroy it was the most thorough of any gate: the shaft was collapsed with the anchor inside it, buried under a mountain of rubble, steel, and frozen ground. The demolition teams reported success. The sensors on the surface report otherwise: **the gate still hums**, through a hundred metres of collapsed ground, and it has not stopped.

## Why It Is Contained

The anchor is **unreachable** — sealed, buried, and reinforced twice since 2031, each time because Denial Command decided that "buried" was not the same as "gone". The collapse zone is a no-man's-land of broken rock and ice; the garrison's maps of what is down there are guesswork, because the shaft's survey was lost with the mine's records.

It is the gate Denial Command worries about most, for the simplest reason: it is the only contained gate that was **deliberately killed and did not die**.

## Holding It

- **The Minehead Garrison** — a reinforced post at the head of the collapsed workings; watchtowers, seismic arrays, and a doctrine of listening.
- The surface is a frozen scar; the garrison's tours are short, the winter is long, and the hum is always there.

## For Story Use

### Themes to Explore
- **The thing that survived being buried** — containment as an act of faith, not engineering.
- The mine as **the grave that was also a door**.

### Story Hooks
- The hum is **unchanged** — the same pitch, the same note, the same volume, through every storm and every reinforcement since 2031. The garrison's engineers stopped reporting it as a fault long ago, because a fault eventually stops. The hum is not a fault. It is a fact.
- A seismograph records a **second source** of vibration under the collapse zone — moving, and moving closer to the surface.
- The mine's original shaft survey is found in a regional archive, misfiled: it shows the shaft going **deeper than any mine in the world has a right to go** — and the last three hundred metres are not stone.

## Related

- [Portals](../lore/portals.md) — how the gates work
- [The Ural Scar](the-ural-scar.md) — the Russian gate that was destroyed; the Mine Gate is the one that was not
- [Joint Portal Command](../factions/joint-portal-command.md) — the authority that holds it
